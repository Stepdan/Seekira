...
public:
    ...
    class ContextArrayIterator
    {
        ...
        public:
            ...
            char* operator[](size_t index) const { return (index < length_) ? *(ptr_ + index) : nullptr; }
        };
    ...
    template <typename T,
              typename = typename std::enable_if<!std::is_base_of<Context, typename std::decay<T>::type>::value>::type>
...
Context::ContextArrayIterator<isConst>::ContextArrayIterator(const ContextArrayIterator& iter)
    : base_(iter.base_)
    , length_(iter.length_)
    , index_(iter.index_)
    , curr_(iter.curr_ ? new Context(*(iter.curr_)) : nullptr)
    , isObj_(iter.isObj_)
    , keys_(iter.keys_)
{
}
...
Context::operator ContextRef() { return ContextRef(handle_); }
...
template <typename T>
typename std::enable_if<!std::is_base_of<Context, typename std::decay<T>::type>::value>::type Context::push_back(
    T&& data)
{
    Context elem;
...
inline bool Context::operator==(const Context& other) const { return this->handle_ == other.handle_; }
...
inline Context::ContextArrayIterator<> Context::begin() { return ContextArrayIterator<>(*this, 0); }

inline Context::ContextArrayIterator<> Context::end() { return ContextArrayIterator<>(*this, -1); }

inline Context::ContextArrayIterator<true> Context::begin() const { return ContextArrayIterator<true>(*this, 0); }

inline Context::ContextArrayIterator<true> Context::end() const { return ContextArrayIterator<true>(*this, -1); }

inline Context::ContextArrayIterator<true> Context::cbegin() const { return ContextArrayIterator<true>(*this, 0); }

inline Context::ContextArrayIterator<true> Context::cend() const { return ContextArrayIterator<true>(*this, -1); }
