...
#include <thirdparty/tdv/modules/ONNXRuntimeAdapter.h>
...
OrtStatus* OnnxRuntimeAdapter::SessionOptionsAppendExecutionProvider_CUDA(OrtSessionOptions* options, int device_id)
{
    ...
    OrtStatus* (*ort_append_cuda_ep)(OrtSessionOptions*, int) =
        (OrtStatus * (*)(OrtSessionOptions*, int)) ONNX_RESOLVE("OrtSessionOptionsAppendExecutionProvider_CUDA");
    RHAssert2(0x9384aad1, ort_append_cuda_ep, GET_ERROR_MSG());
    return ort_append_cuda_ep(options, device_id);
}

OrtStatus* OnnxRuntimeAdapter::SessionOptionsAppendExecutionProvider_Nnapi(OrtSessionOptions* options,
                                                                           uint32_t nnapi_flags)
{
    ...
    OrtStatus* (*ort_append_nnapi_ep)(OrtSessionOptions*) =
        (OrtStatus * (*)(OrtSessionOptions*)) ONNX_RESOLVE("OrtSessionOptionsAppendExecutionProvider_Nnapi");
    RHAssert2(0x4aeea0fc, ort_append_nnapi_ep, GET_ERROR_MSG());
    return ort_append_nnapi_ep(options);
#else
    OrtStatus* (*ort_append_nnapi_ep)(OrtSessionOptions*, uint32_t) =
        (OrtStatus * (*)(OrtSessionOptions*, uint32_t)) ONNX_RESOLVE("OrtSessionOptionsAppendExecutionProvider_Nnapi");
...
OnnxRuntimeAdapter* OnnxRuntimeAdapter::GetInstance(const tdv::data::Context& config)
{
    ...
    if (!use_cuda)
    {
        ...
        if (pinstance_ == nullptr)
        {
            ...
            pinstance_ = new OnnxRuntimeAdapter(
                library_path != config.end() ? (*library_path).get<std::string>() + SLASH + ONNX_NAME : ONNX_NAME,
                use_cuda);
        }
        return pinstance_;
    }
