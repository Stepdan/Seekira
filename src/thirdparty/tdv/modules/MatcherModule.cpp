...
float l2_distance(const float* v1,  // must be aligned for SSE
...
{
    return l2_distance(a["template"].get<std::vector<float>>().data(), b["template"].get<std::vector<float>>().data(),
...
MatcherModule::MatcherModule(const tdv::data::Context& config) : threshold(config.get<double>("threshold", 1.175)) {}
