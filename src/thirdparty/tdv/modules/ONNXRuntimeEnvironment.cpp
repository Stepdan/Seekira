...
ONNXRuntimeEnvironment::ONNXRuntimeEnvironment(const Context& config)
    : ort_api(OnnxRuntimeAdapter::GetInstance(config)->GetApi()), dynamic_batch(false)
{
    ...
    if (config["use_cuda"].get<bool>())
    {
        ...
        OrtCheckStatus(OnnxRuntimeAdapter::GetInstance(config)->SessionOptionsAppendExecutionProvider_CUDA(
    ...
    if (config["use_nnapi"].get<bool>())
    {
#ifdef ONNXRT_OBSOLETE_API
        OrtCheckStatus(
            OnnxRuntimeAdapter::GetInstance(config)->SessionOptionsAppendExecutionProvider_Nnapi(session_options));
#else
        ...
        OrtCheckStatus(OnnxRuntimeAdapter::GetInstance(config)->SessionOptionsAppendExecutionProvider_Nnapi(
    ...
    OrtCheckStatus(ort_api->CreateSessionFromArray(OnnxRuntimeAdapter::GetInstance(config)->GetEnv(), model_buffer,
                                                   model_buffer_size, session_options, &session));
    ...
    for (size_t i = 0; i < numInputNodes; ++i)
    {
        ...
        inputSizes.push_back(
            std::accumulate(std::begin(input_dims), std::end(input_dims), int64_t(1), std::multiplies<int64_t>()));
...
bool ONNXRuntimeEnvironment::adjust_batch_size(size_t input, long batch_size)
{
    if ((input < inputShapes.size()) && dynamic_batch[input])
    {
        if (inputShapes[input][0] != batch_size)
        {
            ...
            inputSizes[input] = std::accumulate(std::begin(inputShapes[input]), std::end(inputShapes[input]),
...
std::shared_ptr<uint8_t> ONNXRuntimeEnvironment::infer(std::vector<void*> input_data)
{
    ...
    for (size_t i = 0; i < inputNames.size(); ++i)
    {
        ...
        OrtCheckStatus(ort_api->CreateTensorWithDataAsOrtValue(
            memory_info, static_cast<uint8_t*>(input_data[i]) + p_data_len, data_size, inputShapes[i].data(),
            inputShapes[i].size(), inputTypes[i], &input_tensor));
        input_tensors.push_back(input_tensor);
        p_data_len += data_size;
    }

    OrtCheckStatus(ort_api->Run(session,                                       // session
    ...
    for (size_t i = 0; i < outputNames.size(); ++i)
    {
        ...
        if (isTensor)
        {
            ...
            outputSizes.push_back(
                std::accumulate(output_dims.begin(), output_dims.end(), int64_t(1), std::multiplies<int64_t>()));
...
const std::vector<std::vector<int64_t>>& ONNXRuntimeEnvironment::getInputShapes() const { return inputShapes; }

const std::vector<std::vector<int64_t>>& ONNXRuntimeEnvironment::getOutputShapes() const { return outputShapes; }

const std::vector<ONNXTensorElementDataType>& ONNXRuntimeEnvironment::getOutputTypes() const { return outputTypes; }
