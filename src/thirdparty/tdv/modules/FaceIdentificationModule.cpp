...
#include <thirdparty/tdv/data/ContextUtils.h>
...
cv::Mat processObject(const tdv::data::Context& obj, const int input_width, const int input_height)
{
    ...
    if (obj.contains("fitter"))
    {
        const cv::Matx23f crop2image =
            makeCrop2ImageByPoints(obj["fitter"], (*image), (std::max)(input_width, input_height));
        cv::Mat result;
    ...
    cv::Rect rect(cv::Point{static_cast<int>(rectCtx[0].get<double>() * img_width),
                            static_cast<int>(rectCtx[1].get<double>() * img_height)},
                  cv::Point{static_cast<int>(rectCtx[2].get<double>() * img_width),
                            static_cast<int>(rectCtx[3].get<double>() * img_height)});
...
void FaceIdentificationModule::preprocess(tdv::data::Context& data)
{
    ...
    else
    {
        ...
        RHAssert2(0x11113333, image.depth() == CV_8U || image.depth() == CV_32F,
                  "only 8U and 32F image types are suported");
    }
