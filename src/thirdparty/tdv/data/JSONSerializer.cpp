...
{
    static void insert(const std::string& /*key*/, Context& ctx, const Type& data) { ctx.push_back(data); }
};
...
{
    Context ctx = json.is_array() ? Context::make_array() : json.is_object() ? Context::make_object() : Context();
