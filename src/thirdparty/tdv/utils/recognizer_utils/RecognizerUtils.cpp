#include <thirdparty/tdv/utils/recognizer_utils/RecognizerUtils.h>
...
void construct_fda_points(const tdv::data::Context& fitter, std::vector<cv::Point2f>& dst_points, int i_w, int i_h)
{
    ...
    dst_points[10] =
        cv::Point2f(fitter["right_eye"][0].get<double>() * i_w, fitter["right_eye"][1].get<double>() * i_h);
}

cv::Matx23f estimate_scaled_rigid_transform(const std::vector<cv::Point2f>& src, const std::vector<cv::Point2f>& dst,
                                            const int iterations_count = 10)
{
    ...
    for (int iterations = 0;; ++iterations)
    {
        ...
        const bool solved = cv::solve(cv::Mat_<float>(4, 4, (float*)sa), cv::Mat_<float>(4, 1, (float*)sb),
                                      cv::Mat_<float>(4, 1, (float*)sr), cv::DECOMP_SVD);
        ...
        std::nth_element(temp.data(), temp.data() + temp.size() / 2, temp.data() + temp.size());
...
cv::Matx23f makeCrop2ImageByPoints(const tdv::data::Context& fitter, cv::Mat& image, const int base_crop_size)
{
    ...
    cv::Point2f crop_points[target_points_count] = {
        cv::Point2f(38.30f, 51.70f), cv::Point2f(73.53f, 51.50f), cv::Point2f(56.00f, 71.70f),
        cv::Point2f(41.55f, 92.37f), cv::Point2f(70.73f, 92.20f),
    };
    ...
    const cv::Point2f image_points[target_points_count] = {
        constructed_points[7],  constructed_points[10], constructed_points[14],
        constructed_points[17], constructed_points[19],
    };

    const cv::Matx23f crop2image =
        estimate_scaled_rigid_transform(std::vector<cv::Point2f>(crop_points, crop_points + target_points_count),
...
void warpAffine(cv::Mat& src, cv::Mat& dst, const cv::Matx23f& transform_m_input_, const cv::Size& dsize)
{
    ...
    if (int_scale == 1)
    {
        cv::warpAffine(src, dst, transform_m, dsize, flags, borderMode, borderValue);
    ...
    cv::Mat tmp;
    cv::warpAffine(src, tmp, transform_m, cv::Size(dsize.width * int_scale, dsize.height * int_scale), flags,
                   borderMode, borderValue);

    cv::resize(tmp, dst, dsize, 0, 0, cv::INTER_AREA);
}
